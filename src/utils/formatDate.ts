 ...
 * 작성자명 : 원종석
 * 작성일자 : 2025-08-28 (월)
 * 작성내용 : ISO 형식의 Date String을 다음과 같은 형태로 변환한다.
 * @param {Date} date 날짜
 * @returns "YYYY-MM-DD HH:mm:ss" 형태의 string 데이터
 */
const setDayjsFormatString = (date?: string): string => {
  return dayjs(date).format("YYYY-MM-DD HH:mm:ss");
};

export { formatDateToString, setKoreaTime, setDayjsFormatString };
