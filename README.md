...
| 담당자 | 업무                            | 비고                                                                                                                               |
| ------ | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 원종석 | API 개발 / 공통 코드 개발       | 필요한 API 개발 / 스키마 및 데이터 타입 정의 / API 호출 공통 함수 개발                                                             |
| 예은선 | MBTI 검사 결과 계산 로직 개발   | 사용자가 선택한 데이터 계산 / 검사 결과 데이터 전달 / 통계 데이터 갱신                                                             |
| 이슬   | 담벼락 글쓰기 컴포넌트 개발     | 사용자가 글을 작성할 수 있는 폼 모달 창 개발 / 제목, 내용 유효성 검사 / MBTI 유형 상태 관리 / 담벼락 스티커 색상 선택 모달 창 개발 |
| 김진규 | MBTI 검사 결과 화면 개발        | 검사 결과 데이터 화면에 표시 / 링크 공유하기 / 유형별 통계 표시(외부 링크로 접근시에는 보이지 않음)                                |
| 엄태호 | MBTI 유형별 결과 통계 화면 개발 | 유형별 결과 통계 차트로 표시 / 문항당 하나의 차트 및 응답을 표시                                                                   |
| 이수민 | MBTI 검사 화면 개발             | 테스트 검사 데이터 스케일링 및 전달                                                                                                |
| 문수민 | 담벼락 화면 개발                | 전체 유형의 글을 화면에 표시 / 유형별 담벼락 페이지에서는 해당 유형의 글만 화면에 표시 / 실시간 좋아요 처리                        |
